...
The testing helpers have more to it than disabling SSL but are not present on NuGet. So naturally, I raised [an issue](https://github.com/Azure/azure-sdk-for-net/issues/30751) to the SDK team if they have any plans in that direction. Since the response was negative, I created a NuGet package inspired by what they did. As demonstrated in the article thus far, many customizations are available. For example, we can run a subset of services and choose HTTPS over HTTP. All these options mean that any helper cannot just assume how Azurite is running.

For this purpose, I created the classes `AzuriteAccount` and `AzuriteAccountBuilder`.  The builder pattern creates an AzuriteAccount. For example the default account can be created like:
